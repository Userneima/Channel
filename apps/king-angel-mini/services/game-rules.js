...
const stageByValue = STAGES.reduce((map, stage) => {
    map[stage.value] = stage;
    return map;
}, {});

const normalizeStage = (stageValue) => (stageByValue[stageValue] ? stageValue : "wish");
const getStage = (stageValue) => stageByValue[normalizeStage(stageValue)];
const getNextStage = (stageValue) => {
    const index = STAGE_ORDER.indexOf(normalizeStage(stageValue));
